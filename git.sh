...
git config --global user.name "Toby Joe Boudreaux"

current_email=$(git config --global --get user.email)

# if current_email is empty, then add an email
if [[ -z "$current_email" ]]
  echo "Current git email is: $current_email ..."
then
  git config --global user.email "tj@tobyjoe.com"
fi

git config --global github.user tobyjoe
